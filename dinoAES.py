...
class dinoAES:
    ...
    def hex2str(self, h):
        return ''.join([chr(i) for i in h])
    
    def en_de_test(self):
        key='dinosaurdinosaur'
        key = self.str2hex(key)
        RoundKeys = self.round_key_generator(key)

        print('>>encrypt')
        plaintext = 'DinosaurDinosaur'
        print('plaintext : ' + plaintext)
        plaintext = self.str2hex(plaintext)
        print('plaintext = ', hex(plaintext))
        plaintext = self.num_2_16bytes(plaintext)
        ciphertext = self.aes_encrypt(plaintext, RoundKeys)
        print('ciphertext = ' + hex(self._16bytes2num(ciphertext)))

        print('>>decrypt')
        #ciphertext = 0xb02ac35183d2d15e904c91dba1195d21
        #ciphertext = self.num_2_16bytes(ciphertext)
        plaintext = self.aes_decrypt(ciphertext, RoundKeys)
        print('plaintext = ' + hex(self._16bytes2num(plaintext)))
        plaintext = self.hex2str(plaintext)
        print('plaintext : ', plaintext)

    def test_encrypt_diffusion(self):
        key='dinosaurdinosaur'
        key = self.str2hex(key)
        RoundKeys = self.round_key_generator(key)

        plaintext = 'DinosaurDinosaur'
        print('pre-plaintext : ' + plaintext)
        plainnum = self.str2hex(plaintext)
        print('pre-plaintext = ', hex(plainnum))
        count = 2**8
        total_changed = 0
        for testcase in range(0,count):
            test=plainnum+testcase
            a=self.aes_encrypt(self.num_2_16bytes(test), RoundKeys)
            if(test&1==1):
                b=self.aes_encrypt(self.num_2_16bytes(test-1), RoundKeys)
            else:
                b=self.aes_encrypt(self.num_2_16bytes(test+1), RoundKeys)
            a=self._16bytes2num(a)
            b=self._16bytes2num(b)
            xor=a^b
            changed=0
            while xor>0:
                changed += xor&1
        ...
        print(total_changed/count) 

    def test_sbox_diffusion(self):
        count = 2**16
        total_changed = 0
        for testcase in range(0,count):
            a=self.SubWord(testcase)
            if(testcase&1==1):
                b=self.SubWord(testcase-1)
            else:
                b=self.SubWord(testcase+1)
            xor=a^b
            changed=0
    ...
    def test_sbox_cycle(self):
        for case in range(0,0xff):
            testcase=case
            result = 1
            i = testcase>>4& 0xf
            j = testcase& 0xf
            testcase= self.S_BOX[i][j]
            while testcase!=0x00:
                i = testcase>>4& 0xf
                j = testcase& 0xf
                testcase= self.S_BOX[i][j]
                ...
                if result>1e6:
                    result='__'
                    break
            print(hex(case) ,result,end='|')
            if (case+1)%10==0:print()
    
    def round(self, plaintext_list, RoundKeys,Round):
        State = plaintext_list
        State = self.SubBytes(State)
        State = self.ShiftRows(State)
        State = self.MixColumns(State)
    ...
    def Round_diffusion(self):
        ...
        plainnum = self.str2hex(plaintext)
        print('input = ', hex(plainnum))
        count = 2**10
        total_changed = 0
        for testcase in range(0,count):
            for r in range (1,10):
                test=plainnum+testcase
                a=self.round(self.num_2_16bytes(test), RoundKeys,r)
                if(test&1==1):
                    b=self.round(self.num_2_16bytes(test-1), RoundKeys,r)
                else:
                    b=self.round(self.num_2_16bytes(test+1), RoundKeys,r)
                a=self._16bytes2num(a)
                b=self._16bytes2num(b)
                ...
                while xor>0:
                    ...
                    xor>>=1
                total_changed+=changed
        print(total_changed/(9*count))

if __name__ == '__main__':
    myaes = dinoAES()
    #myaes.en_de_test()
    #myaes.test_encrypt_diffusion()
    #myaes.test_sbox_diffusion()
    #myaes.test_sbox_cycle()
    #myaes.Round_diffusion()
