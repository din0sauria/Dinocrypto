...
def encry_sm2(args, PB, M):
    ...
    klen = 4 * len(M_hex)
    #("明文消息的比特串长度klen是：", klen)
    t = KDF(x2_bits + y2_bits, klen)
    #("通过KDF算法计算得到的t=KDF(x2 ∥ y2, klen)是：", t)
    t_hex = bits_to_hex(t)
    #("t的十六进制表示形式是：", t_hex)
    #("\nA6：计算计算C2 = M ⊕ t")
...
print("256位素数域上的椭圆曲线。椭圆曲线方程为：","y^2 = x^3 + ax + b")
