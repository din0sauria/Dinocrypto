...
class dinoDES():
    def __init__(self):
        ...
        # 设置默认密钥
        self.K = '0110010001101001011011100110111101110011011000010111010101110010'#dinosaur
        self.k1 = [
            57, 49, 41, 33, 25, 17, 9,
            1, 58, 50, 42, 34, 26, 18,
            10, 2, 59, 51, 43, 35, 27,
    ...
    def getchange(self, table: str, self_table: list) -> str:
        """
    ...
    def bin2int(self, binary: str) -> list:
        """
    ...
    def get_block_list(self, binary: str) -> list:
        """
    ...
    def __f_funtion(self, right: str, key: str):
        ...
        """
        # 对right进行E扩展
        e_result = self.getchange(right, self.E)
        # 与key 进行异或操作
        xor_result = self.xor_function(e_result, key)
        # 进入S盒子
        s_result = self.s_box(xor_result)
        # 进行P置换
        p_result = self.getchange(s_result, self.P)
        return p_result
 
    def get_subkey_list(self):
        """
        :return: 返回加密过程中16轮的子密钥
        """
        key = self.getchange(self.K, self.k1)
        left_key = key[0:28]
        right_key = key[28:56]
        keys = []
        for i in range(1, 17):
            ...
            ki = self.getchange(move_key, self.k2)
            keys.append(ki)
        return keys
 
    def xor_function(self, xor1: str, xor2: str):
        """
        :param xor1: 01字符串
        :param xor2: 01字符串
        :return: 异或操作返回的结果
    ...
    def iteration(self, bin_plaintext: str, key_list: list):
        ...
        for i in range(0, 16):
            ...
            next_right = self.xor_function(left, f_result)
            left = next_lift
            right = next_right
    ...
    def encode(self, plaintext):
        ...
        bin_plaintext_block = self.get_block_list(bin_plaintext)
        ciphertext_bin_list = []
        key_list = self.get_subkey_list()
        for block in bin_plaintext_block:
            # 初代ip置换
            sub_ip = self.getchange(block, self.ip)
            ite_result = self.iteration(sub_ip, key_list)
            # 逆ip置换
            sub_ip1 = self.getchange(ite_result, self.ip1)
            ciphertext_bin_list.append(sub_ip1)
        ciphertext_bin = ''.join(ciphertext_bin_list)
        result = self.bin2int(ciphertext_bin)
    ...
    def decode(self, ciphertext):
        ...
        for block in bin_ciphertext_block:
            ...
            sub_ip1 = self.getchange(ite, self.ip1)
    ...
    def en_de_test(self):
        plaintext = "chenyidong"
        key="dinosaur"#8字节64位
        bin_key = self.str2bin(key)
        self.K = bin_key
        print("The key is:{}".format(key))
        print("The bin of key is:{}".format(bin_key))
        print("The plaintext is:{}".format(plaintext))
        ciphertext = self.encode(plaintext)
        print("The ciphertext is:{}".format(ciphertext))
        plaintext = self.decode(ciphertext)
        print("The decoded result is:{}".format(plaintext))
    
    def test5(self):
        plaintext1 = "whua"
        print("The bin of plaintext is:{}".format(self.str2bin(plaintext1)))
        plaintext3 = "whuc"
        print("The bin of plaintext is:{}".format(self.str2bin(plaintext3)))
        key="dinosaur"#8字节64位
        bin_key = self.str2bin(key)
        self.K = bin_key
        ciphertext = self.encode(plaintext1)
        print("The ciphertext is:{}".format(ciphertext))
        ciphertext = self.encode(plaintext3)
        print("The ciphertext is:{}".format(ciphertext))
    ...
    def test5_2(self):
        ...
        for x in xs:
            for i in range(8):
                block1 = x
                block2 = self.xor_function(x,'001100')
                line = int(block1[0] + block1[5], 2)
                colmn = int(block1[1:5], 2)
                res = bin(self.S[i][line*16 + colmn])[2:]
                if len(res) < 4:
                    res = '0' * (4 - len(res)) + res
                print(res,end=' ')
                line = int(block2[0] + block2[5], 2)
                colmn = int(block2[1:5], 2)
                res = bin(self.S[i][line*16 + colmn])[2:]
                if len(res) < 4:
                    res = '0' * (4 - len(res)) + res
                print(res)
    ...
    def test5_3(self):
        xs=['101010']
        xor_list=['110000','110100','111000','111100']
        for x in xs:
            for i in range(8):
                block1 = x
                line = int(block1[0] + block1[5], 2)
                ...
                print('test:',res)
                for xor in xor_list:
                    block2 = self.xor_function(x,xor)
                    line = int(block2[0] + block2[5], 2)
                    colmn = int(block2[1:5], 2)
                    res = bin(self.S[i][line*16 + colmn])[2:]
                    if len(res) < 4:
    ...
    def test5_5(self):
        ...
        all_resault =''
        for i in range(2**5):
            x=self.xor_function(bin(i)[:5],'000000')
            while len(x) < 5: x = '0'+x
            set1.append(x+'0')
        print(set1)
        for x in set1:
